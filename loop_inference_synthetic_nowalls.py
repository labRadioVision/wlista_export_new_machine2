# -*- coding: utf-8 -*-
"""
loop_inference_synthetic_nowalls.py
====================================
SOLO INFERENZA sul caso sintetico nowalls (PEC, free-space) — stessa coppia
di file di run_wlista_synthetic_nowalls_gpu.py:
  - E_total_freespace_nowalls.mat (incidente)
  - E_total_Ken_PEC_nowalls.mat   (totale, 11 posizioni)

E' l'equivalente di loop_inference_ken_grasso.py adattato a questo dataset
(nessun override di ETOTAL_FILE/SYNTH_DIR: il modulo base
run_wlista_synthetic_nowalls.py punta già di default a questa coppia di
file). Dato un prefisso di checkpoint, cicla su TUTTE le epoche disponibili
(`<prefix>_epNNN.pt`, escluso `<prefix>_best.pt`) e su TUTTE le 11
posizioni, eseguendo MF + ISTA + modello (LISTA / W-LISTA / LR-W-LISTA,
auto-detect).
...
Inoltre salva metrics.csv con, per ogni riga: epoch, pos_label,
data_consistency, signal_clutter, mse_full, mse_occ, mse_bg, checkpoint.

Uso
---
  python3 loop_inference_synthetic_nowalls.py --prefix wlista_synthetic_nowalls_gpu \\
      --ckpt-dir checkpoints_lista

  python3 loop_inference_synthetic_nowalls.py --prefix lista_synthetic_nowalls_gpu \\
      --ckpt-dir checkpoints_lista

  python3 loop_inference_synthetic_nowalls.py --prefix wlista_lowrank_phased_nowalls_r8_A \\
      --ckpt-dir checkpoints_lista_lowrank_phased_synthetic_nowalls
...
# E_total_freespace_nowalls.mat). Per l'inferenza vogliamo TUTTE le 11
# posizioni (non lo split train/val usato in training).
# ---------------------------------------------------------------------------
_n_pos = len(base.POS_LABELS)
base.TRAIN_IDX = list(range(_n_pos))   # tutte le posizioni
base.VAL_IDX   = []                     # non serve qui: prendiamo tutto da TRAIN_IDX
print(f"[synthetic_nowalls inference] N_pos = {_n_pos}")

OUT_ROOT = os.path.join(SCRIPT_DIR, "results_inference_synthetic_nowalls")


def guess_ckpt_dir(prefix):
    if "phased" in prefix:
        return os.path.join(SCRIPT_DIR, "checkpoints_lista_lowrank_phased_synthetic_nowalls")
    if "wfirst" in prefix:
        return os.path.join(SCRIPT_DIR, "checkpoints_lista_lowrank_wfirst_synthetic_nowalls")
    if "lowrank" in prefix:
        return os.path.join(SCRIPT_DIR, "checkpoints_lista_lowrank_synthetic_nowalls")
    # LISTA / W-LISTA "plain" (run_wlista_synthetic_nowalls_gpu.py)
    return os.path.join(SCRIPT_DIR, "checkpoints_lista")
...
def main():
    ...
    ap.add_argument("--prefix", required=True,
                    help="prefisso checkpoint, es. wlista_synthetic_nowalls_gpu / "
                         "lista_synthetic_nowalls_gpu / wlista_lowrank_phased_nowalls_r8_A")
    ...
    t_start = time.time()
    print("=" * 70)
    print(f"INFERENCE SWEEP synthetic_nowalls  prefix={args.prefix}  device={args.device}")
