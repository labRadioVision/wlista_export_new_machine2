# -*- coding: utf-8 -*-
"""
run_wlista_lowrank_phased_synthetic_nowalls.py
===============================================
LR-W-LISTA "phased" sul caso sintetico nowalls (PEC, free-space) — stessa
coppia di file usata da run_wlista_synthetic_nowalls_gpu.py:
  - E_total_freespace_nowalls.mat (incidente)
  - E_total_Ken_PEC_nowalls.mat   (totale, 11 posizioni)

E' l'equivalente di run_wlista_lowrank_phased_ken_grasso.py adattato a questo
dataset (nessun override di ETOTAL_FILE/SYNTH_DIR: il modulo base
run_wlista_synthetic_nowalls.py punta già di default a questa coppia di
file). Split TRAIN/VAL allineato a run_wlista_synthetic_nowalls_gpu.py
(tutte le 11 posizioni in training, pos 7 come validation/riferimento) per
poter confrontare direttamente i risultati con LISTA/W-LISTA su questo caso.

Differenza chiave rispetto a un eventuale wfirst: dopo la fase di training
di W, W (log_wx/wy/wz) e log_lambda vengono CONGELATI — si allenano solo
U, V (+ log_mu). Più stabile: evita il collasso di z osservato con W che
continua ad allenarsi insieme a UV (vedi SESSION_LOG.md).
...
---
    ...
    # salta fase A (riparti da un checkpoint di fase A gia' fatto)
    python3 run_wlista_lowrank_phased_synthetic_nowalls.py \\
        --skip-a checkpoints_lista_lowrank_phased_synthetic_nowalls/wlista_lowrank_phased_nowalls_r8_A_best.pt

    # salta fase A+B (riparti da un checkpoint di fase B gia' fatto)
    python3 run_wlista_lowrank_phased_synthetic_nowalls.py \\
        --skip-b checkpoints_lista_lowrank_phased_synthetic_nowalls/wlista_lowrank_phased_nowalls_r8_B_best.pt

I file E_total_freespace_nowalls.mat e E_total_Ken_PEC_nowalls.mat devono
essere in Dataset TUM/sinthetic_data/ (sottocartella della cartella in cui
si trova questo script) — sono già bundled, nessun setup aggiuntivo.
"""
...
# ---------------------------------------------------------------------------
# Dataset: usa i default del modulo base (E_total_Ken_PEC_nowalls.mat +
# E_total_freespace_nowalls.mat) — nessun override di ETOTAL_FILE/SYNTH_DIR.
# Split allineato a run_wlista_synthetic_nowalls_gpu.py: tutte le 11
# posizioni in training, pos 7 come validation/riferimento.
# ---------------------------------------------------------------------------
base.TRAIN_IDX = list(range(11))
base.VAL_IDX   = [7]

# ---------------------------------------------------------------------------
# Cartelle output dedicate
# ---------------------------------------------------------------------------
OUT_DIR  = os.path.join(SCRIPT_DIR, "results_wlista_lowrank_phased_synthetic_nowalls")
CKPT_DIR = os.path.join(SCRIPT_DIR, "checkpoints_lista_lowrank_phased_synthetic_nowalls")
...
# LR ridotti (stesso fix applicato a run_wlista_synthetic_nowalls_gpu.py /
# run_*_ken_grasso.py: base.LR=5e-2 / base.LR_W=5e-1 erano troppo alti,
# causa probabile del collasso di z osservato in wfirst)
LR_BASE   = 1e-2
LR_W_BASE = 1e-1
...
if __name__ == "__main__":
    ...
    t_start = time.time()
    print("=" * 68)
    print(f"LR-W-LISTA PHASED (W allenato da zero, poi congelato) synthetic nowalls — rank={RANK}  device={DEVICE}")
