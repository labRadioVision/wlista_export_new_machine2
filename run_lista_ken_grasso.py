# -*- coding: utf-8 -*-
"""
run_lista_ken_grasso.py
========================
LISTA baseline (senza pesi per-asse, senza correzione low-rank) su
E_total_Ken_grasso_nowalls.mat. Caso base per confronto con W-LISTA
(run_wlista_ken_grasso.py) e LR-W-LISTA (run_lowrank_ken_grasso.py /
run_wlista_lowrank_wfirst_ken_grasso.py).

Run
---
    nohup python3 run_lista_ken_grasso.py > lista_ken_grasso.log 2>&1 &
    python3 run_lista_ken_grasso.py --resume checkpoints_lista_plain_ken_grasso/lista_ken_grasso_ep005.pt
...
print(f"[Ken_grasso LISTA] N_pos rilevato = {_n_pos}  (chiave: '{_k}')")
...
# ---------------------------------------------------------------------------
OUT_DIR  = os.path.join(SCRIPT_DIR, "results_lista_plain_ken_grasso")
CKPT_DIR = os.path.join(SCRIPT_DIR, "checkpoints_lista_plain_ken_grasso")
...
# ---------------------------------------------------------------------------
# Iperparametri (stessi K/epochs/lr del baseline W-LISTA, niente lr_w/clamp)
# ---------------------------------------------------------------------------
K           = base.K
...
def train(op, b_tr, z_tr, b_va, z_va, ckpt_name, resume=None):
    model = LISTAHolography(K=K, L_est=L_EST, lambda_init=LAMBDA_INIT).to(DEVICE)
    ...
    print(f"\n[LISTA Ken_grasso] K={K} epochs={start_epoch}->{N_EPOCHS} "
          f"N_train={N} N_val={len(b_va)} device={DEVICE}")
    print(f"  #params={sum(p.numel() for p in model.parameters())}  lr={LR:.1e}")
    ...
    for epoch in range(start_epoch, N_EPOCHS + 1):
        ...
        ckpt = dict(epoch=epoch, K=K, model_type="lista",
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    ckpt_name = "lista_ken_grasso"

    t_start = time.time()
    print("=" * 68)
    print(f"LISTA (plain) Ken_grasso_nowalls  device={DEVICE}")
    ...
    if args.infer_only:
        ...
        model = LISTAHolography(K=int(ck["K"]), L_est=L_EST, lambda_init=LAMBDA_INIT)
